  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-[#13121A] p-6 rounded-xl border border-slate-800/60 shadow-lg flex flex-col">
          ...
          <div className="text-3xl font-bold text-white mb-1">{stats?.views || 0}</div>
          <div className="text-sm text-slate-500 font-medium">Total Page Views</div>
        </div>
        ...
        <div className="bg-[#13121A] p-6 rounded-xl border border-slate-800/60 shadow-lg flex flex-col">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="text-xs font-semibold text-slate-500 bg-slate-800 px-2 py-1 rounded-full border border-slate-700">All time</span>
          </div>
          <div className="text-3xl font-bold text-white mb-1">₹{stats?.revenue?.toLocaleString() || 0}</div>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-white">My Stores</h2>
        </div>
        <div className="bg-[#13121A] rounded-xl border border-slate-800/60 shadow-lg overflow-hidden">
          {websites && websites.length > 0 ? (
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-800/60 text-sm">
                  {websites.map((website, index) => {
                    ...
                    return (
                      <tr key={website._id} className="hover:bg-slate-800/20 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-bold text-white">{website.storeName || `Store ${index + 1}`}</div>
                              <div className="text-xs text-slate-500 capitalize">{website.storeInfo?.category || 'General'}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <div className="relative inline-block text-left">
                              <button
                                ...
                                className="p-2 hover:bg-slate-800 rounded-lg text-slate-500 hover:text-slate-300 transition-colors border border-transparent hover:border-slate-700"
                                title="Actions"
                              >
                                <FaEllipsisV />
                              </button>
                              {isOpen && (
                                <>
                                  ...
                                  <div className="absolute right-0 mt-1 w-44 bg-[#13121A] border border-slate-700/60 rounded-xl shadow-2xl shadow-black/50 z-40 py-1.5 text-left text-xs">
                                    <button
                                      onClick={() => { window.open(`/website/${website.slug}`, '_blank'); setActiveMenuId(null); }}
                                      className="w-full px-4 py-2 hover:bg-slate-800/60 text-slate-300 font-semibold flex items-center gap-2"
                                    >
                                      <FaEye className="text-slate-500" /> View Live Site
                                    </button>
                                    <button
                                      onClick={() => { setSelectedWebsite(website); navigate('settings'); setActiveMenuId(null); }}
                                      className="w-full px-4 py-2 hover:bg-slate-800/60 text-slate-300 font-semibold flex items-center gap-2"
                                    >
                                      <FaCog className="text-slate-500" /> Store Settings
                                    </button>
            ...
            <div className="p-12 text-center">
              ...
              <p className="font-semibold text-white mb-1">No stores yet</p>
              <p className="text-slate-500 text-sm">Create your first store from the Templates section!</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-[#13121A] rounded-xl border border-slate-800/60 shadow-lg overflow-hidden">
            {recentOrders && recentOrders.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-sm text-slate-500 mt-1">When customers place orders, they'll appear here.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-900/60 border-b border-slate-800/60 text-xs uppercase text-slate-500 tracking-wider">
                  ...
                  <tbody className="divide-y divide-slate-800/60 text-sm">
                    {recentOrders && recentOrders.map(order => (
                      <tr key={order._id} className="hover:bg-slate-800/20 transition-colors">
                        <td className="px-6 py-4 font-mono font-bold text-slate-300">#{order._id.substring(order._id.length - 6).toUpperCase()}</td>
