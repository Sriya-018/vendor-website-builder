  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold text-white">Select a Store to Design</h2>
          <p className="text-sm text-slate-400">Choose which store you want to open in the Advanced Website Editor.</p>
        </div>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {websites && websites.length > 0 ? websites.map((website, index) => (
          <div key={website._id} className="bg-[#13121A] p-5 rounded-xl border border-slate-800/60 shadow-lg hover:shadow-xl hover:border-purple-500/40 transition-all cursor-pointer flex flex-col" onClick={() => routerNavigate(`/editor/${website._id}`)}>
            <div className="flex justify-between items-start mb-3">
              <div className="font-bold text-white truncate">{website.storeName || `Store ${index + 1}`}</div>
            ...
            <div className="text-sm text-slate-400 mb-4 line-clamp-2">
              ...
              Slug: <span className="font-medium text-slate-350">{website.slug}</span>
            </div>
          ...
          <div className="col-span-full bg-[#13121A] rounded-xl border border-slate-800/60 shadow-lg overflow-hidden p-12 text-center">
            ...
            <p className="text-slate-400 mb-6">You don't have any stores yet. Create one to edit its design.</p>
          </div>
        )}
      </div>
    </div>
