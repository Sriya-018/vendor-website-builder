            ...
            <header className={`relative flex items-center overflow-hidden transition-all ${theme.heroClass || 'min-h-[80vh] bg-slate-900 text-white'}`}>
              ...
              {isEditable && (
                <button 
                  ...
                  className="absolute bottom-6 right-6 z-20 px-4 py-2 bg-white text-slate-800 rounded-full font-bold text-xs shadow-md hover:scale-105 transition-transform flex items-center gap-1.5 font-sans"
                >
                ...
                <div className="max-w-2xl" style={{ textAlign: config.header?.heroAlign || 'left' }}>
                  ...
                  <div className={`flex flex-wrap gap-4 ${config.header?.heroAlign === 'center' ? 'justify-center' : config.header?.heroAlign === 'right' ? 'justify-end' : ''}`}>
                    <button 
                      ...
                      className={`px-8 py-4 font-bold hover:opacity-90 transition-opacity flex items-center gap-2 text-sm uppercase tracking-wider ${theme.ctaButtonClass || 'bg-blue-600 text-white rounded-lg'}`}
      ...
      case 'products':
        return (
          <SectionWrapper key="products" isEditable={isEditable} sectionKey="products" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section className={`py-24 px-8 ${theme.catalogSectionClass || 'bg-white text-slate-900'}`}>
              <div className="max-w-7xl mx-auto">
                <div className="text-center mb-16">
                  <h2 className={`font-black tracking-tight ${theme.sectionHeadingClass || 'text-3xl md:text-4xl text-slate-800'}`}>
                ...
                <div className="grid md:grid-cols-4 gap-8">
                  {products.slice(0, 4).map((p, i) => (
                    <div key={p.id} className={`group flex flex-col justify-between h-full p-4 border transition-all ${theme.productCardClass || 'bg-white rounded-xl border-slate-100 shadow-sm hover:shadow-md'}`}>
                      <div className="relative aspect-square overflow-hidden rounded-lg bg-slate-100 mb-4">
                        ...
                        {p.isBestseller && (
                          <span className="absolute top-2 left-2 bg-amber-500 text-white text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full">Bestseller</span>
                        )}
                        {!p.inStock && (
                          <span className="absolute top-2 right-2 bg-red-600 text-white text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full">Sold Out</span>
                        )}
                      </div>
                      <div className="flex-1 flex flex-col">
                        <h3 className="font-bold text-base mb-1 line-clamp-1">{p.name}</h3>
                        <p className="text-sm text-slate-500 line-clamp-2 leading-relaxed flex-1 mb-3">{p.description}</p>
                        <div className="flex items-center justify-between mb-4">
                          <span className="font-black text-lg text-slate-800">₹{p.price}</span>
                          <span className="text-xs text-slate-400 font-medium">Qty: {p.stockQuantity}</span>
                        </div>
                        <button
                          className={`w-full py-2.5 text-xs font-bold uppercase tracking-wider rounded-lg transition-colors flex items-center justify-center gap-2 ${
                            p.inStock 
      ...
      case 'gallery':
        ...
        return (
          <SectionWrapper key="gallery" isEditable={isEditable} sectionKey="gallery" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section className={`py-24 px-8 ${theme.gallerySectionClass || 'bg-slate-50'}`}>
              <div className="max-w-7xl mx-auto">
                <div className="text-center mb-16">
                  <h2 className={`font-black tracking-tight ${theme.sectionHeadingClass || 'text-3xl text-slate-800'}`}>{galleryTitle}</h2>
                  <div className="w-12 h-0.5 mx-auto mt-4" style={{ backgroundColor: accentColor }}></div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                  {galleryImages.map((img, idx) => (
                    <div key={idx} className="aspect-square overflow-hidden rounded-xl bg-slate-200 shadow-sm border border-slate-100">
      ...
      case 'faq':
        ...
        return (
          <SectionWrapper key="faq" isEditable={isEditable} sectionKey="faq" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section className={`py-24 px-8 ${theme.faqSectionClass || 'bg-white'}`}>
              <div className="max-w-3xl mx-auto">
                <div className="text-center mb-16">
                  <h2 className={`font-black tracking-tight ${theme.sectionHeadingClass || 'text-3xl text-slate-800'}`}>{faqTitle}</h2>
                  <div className="w-12 h-0.5 mx-auto mt-4" style={{ backgroundColor: accentColor }}></div>
                </div>
                <div className="space-y-4">
                  {faqList.map((faq, idx) => (
                    <div key={faq.id} className="border border-slate-100 rounded-xl overflow-hidden shadow-sm bg-white">
                      <button
                        onClick={() => setActiveFaq(activeFaq === idx ? null : idx)}
                        className="w-full px-6 py-5 flex items-center justify-between text-left font-bold text-slate-700 hover:text-slate-900 transition-colors"
                      >
      ...
      case 'testimonials':
        ...
        return (
          <SectionWrapper key="testimonials" isEditable={isEditable} sectionKey="testimonials" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section className={`py-24 px-8 ${theme.testimonialsSectionClass || 'bg-slate-50'}`}>
              <div className="max-w-7xl mx-auto">
                <div className="text-center mb-16">
                  <h2 className={`font-black tracking-tight ${theme.sectionHeadingClass || 'text-3xl text-slate-800'}`}>{testTitle}</h2>
                  <div className="w-12 h-0.5 mx-auto mt-4" style={{ backgroundColor: accentColor }}></div>
                </div>
                <div className="grid md:grid-cols-3 gap-8">
                  {testList.map((item, idx) => (
                    <div key={item.id || idx} className={`p-8 bg-white border border-slate-100 rounded-2xl shadow-sm flex flex-col justify-between ${theme.testimonialCardClass || ''}`}>
                      <p className="text-slate-600 text-sm leading-relaxed italic mb-6">“{item.review || item.text}”</p>
                      <div className="border-t border-slate-50 pt-4 flex items-center justify-between">
                        <div>
                          <h4 className="font-bold text-slate-800 text-sm">{item.name || item.author}</h4>
      ...
      case 'hours':
        ...
        return (
          <SectionWrapper key="hours" isEditable={isEditable} sectionKey="hours" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section className={`py-24 px-8 ${theme.hoursSectionClass || 'bg-white'}`}>
              <div className="max-w-xl mx-auto text-center">
                <h2 className={`font-black tracking-tight mb-8 ${theme.sectionHeadingClass || 'text-3xl text-slate-800'}`}>{hoursTitle}</h2>
                <div className="divide-y divide-slate-100 border-t border-b border-slate-100 py-4 text-sm">
      ...
      case 'contact':
        ...
        return (
          <SectionWrapper key="contact" isEditable={isEditable} sectionKey="contact" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section className={`py-24 px-8 ${theme.contactSectionClass || 'bg-slate-50'}`}>
              <div className="max-w-4xl mx-auto">
                <div className="grid md:grid-cols-2 gap-16">
                  <div>
                    <h2 className={`font-black tracking-tight mb-6 ${theme.sectionHeadingClass || 'text-3xl text-slate-800'}`}>{contactTitle}</h2>
                    <p className="text-slate-500 mb-8 leading-relaxed">Have questions or want custom orders? Message us directly using the form, or reach out using the contact details below.</p>
                    <div className="space-y-4">
                      {phoneNumber && (
                  ...
                  <form onSubmit={handleContactSubmit} className="bg-white p-8 border border-slate-100 rounded-2xl shadow-sm space-y-4">
                    <div>
                    ...
                    <button
                      type="submit"
                      className="w-full py-3.5 font-bold uppercase tracking-wider text-xs rounded-lg text-white shadow-sm flex items-center justify-center gap-2"
                      style={{ backgroundColor: accentColor }}
                    >
                      <FaPaperPlane size={10} /> Send Message
                    </button>
                    {formSubmitted && (
                      <p className="text-xs text-green-600 font-bold text-center">Thank you! Your message was submitted successfully.</p>
                    )}
                  </form>
                </div>
              </div>
  ...
  if (currentPage === 'shop') {
    return (
      <div 
        ...
        className={`w-full h-full overflow-y-auto relative ${theme.layoutClass || 'bg-white text-slate-900'} ${theme.fontClass || 'font-sans'}`}
      >
        {/* Navigation */}
        <nav className={`sticky top-0 left-0 right-0 z-50 transition-all py-5 px-8 ${theme.navbarClass || 'bg-white border-b border-slate-100 shadow-sm'}`}>
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            {isEditable ? (
              fullLogoUrl ? (
                <div className={`font-black uppercase tracking-tight text-xl ${theme.logoTextClass || 'text-slate-800'}`}>
                ...
                <EditableText
                  ...
                  tagName="span"
                  className={`font-black focus:outline-none uppercase tracking-tight text-xl cursor-text ${theme.logoTextClass || 'text-slate-800'}`}
                />
              )
            ) : (
              <button onClick={() => changePage('home')} className={`font-black focus:outline-none uppercase tracking-tight text-xl ${theme.logoTextClass || 'text-slate-800'}`}>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 border-b border-slate-100 pb-8">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-2">Browse our high-quality inventory.</p>
              </div>
              ...
              <div className="flex flex-wrap gap-4 items-center">
                <div className="relative">
                  <input
                    ...
                    className="pl-10 pr-4 py-2 text-xs border border-slate-200 rounded-lg focus:outline-none w-64"
                  />
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="border border-slate-200 rounded-lg py-2 px-3 text-xs bg-white text-slate-700 focus:outline-none"
                  >
        ...
        {/* Footer */}
        <footer className={`py-12 border-t border-slate-100 text-center text-xs text-slate-400 font-light ${theme.footerClass || 'bg-white'}`}>
    ...
    >
      ...
      {showBgModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl relative text-left text-slate-800 border border-slate-150">
            <button 
              onClick={() => setShowBgModal(false)}
              className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 font-bold text-2xl w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 transition-colors"
            >
              ×
            </button>
            ...
            <div className="flex border-b border-slate-200 mb-6">
              <button 
                ...
                className={`flex-1 pb-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'presets' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
              >
                Category Presets
              </button>
              <button 
                onClick={() => setActiveTab('ai')}
                className={`flex-1 pb-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'ai' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
              >
                AI Generator
              </button>
            </div>
            ...
            {activeTab === 'presets' ? (
              <div className="space-y-4">
                <p className="text-xs text-slate-500 mb-4">Choose a curated, high-resolution aesthetic background preset for your niche:</p>
                <div className="grid grid-cols-2 gap-4 max-h-[250px] overflow-y-auto pr-1">
                  {presets.map((preset, idx) => (
                    <button
                      ...
                      className="group relative aspect-video rounded-xl overflow-hidden bg-slate-100 border border-slate-100 hover:scale-[1.02] transition-transform duration-200 text-left"
                    >
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">AI Generation Prompt</label>
                  <textarea 
                    value={aiPrompt}
                    onChange={(e) => setAiPrompt(e.target.value)}
                    placeholder="e.g. A gorgeous luxury cosmetics counter with glowing lights and serum bottles, photo realistic..."
                    className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-slate-400 bg-slate-50/50"
                    rows="3"
                  ></textarea>
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Artistic Style</label>
                ...
                <button
                  ...
                  className="w-full mt-4 py-3.5 font-bold uppercase tracking-wider text-xs bg-slate-900 text-white rounded-xl hover:bg-slate-800 shadow-sm flex items-center justify-center gap-2"
