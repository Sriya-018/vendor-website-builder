...
import { 
  ...
  FaSearch, FaFilter, FaSortAmountDown, FaHome
} from 'react-icons/fa';
import EditableText from '../EditableText';
import SectionWrapper from '../SectionWrapper';
import CustomPageRenderer from './CustomPageRenderer';

export default function TemplateLoft({ 
  config, 
  business, 
  products, 
  devicePreview, 
  ...
  // Custom Loft specific state: Shop by Room filter
  ...
  const businessName = config.navbar?.logoText || (website?.storeInfo?.businessName ?? business?.businessName ?? 'Nordic Loft');
  ...
  const description = config.header.heroHeading || 'Scandinavian minimalist furniture and clean designs.';
  ...
  const getProductImageUrl = (product, i) => {
    ...
    if (!imgUrl) return `https://picsum.photos/seed/loft${i}/600/600`;
  ...
  // Filter featured products by room setting tab
  const activeLoftProducts = products.filter(p => {
    if (activeRoomTab === 'All') return true;
    return (p.category || '').toLowerCase() === activeRoomTab.toLowerCase();
  });
  ...
  const renderSection = (key) => {
    ...
    switch (key) {
      case 'hero':
        return (
          <SectionWrapper key="hero" isEditable={isEditable} sectionKey="hero" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <header className="relative min-h-[75vh] flex items-center overflow-hidden animate-fade-in bg-[#f4f5f4]">
              <div className="absolute inset-0 z-0">
                <img src={heroImage} className="w-full h-full object-cover opacity-85" alt="Hero" />
                <div className="absolute inset-0" style={{ background: 'linear-gradient(90deg, rgba(46,48,47,0.85) 30%, rgba(46,48,47,0.2))' }}></div>
              </div>
              ...
              <div className="relative z-10 max-w-7xl mx-auto px-8 w-full">
                <div className="max-w-lg" style={{ textAlign: config.header.heroAlign }}>
                  <p className="text-xs font-bold tracking-[0.25em] uppercase mb-4" style={{ color: accentColor }}>Nordic Simplicity</p>
                  <EditableText
                    isEditable={isEditable}
                    value={config.header.heroHeading || storeName}
                    onChange={(val) => onUpdateConfig('header', 'heroHeading', val)}
                    tagName="h1"
                    className="text-4xl md:text-6xl font-bold leading-tight mb-5 text-[#f8fafc]"
                  />
                  <EditableText
                    isEditable={isEditable}
                    ...
                    tagName="p"
                    className="text-md text-slate-350 mb-10 leading-relaxed font-light"
                  />
      ...
      case 'products':
        return (
          <SectionWrapper key="products" isEditable={isEditable} sectionKey="products" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section id="products" className="max-w-7xl mx-auto px-8 py-24 text-[#2E302F]">
              
              <div className="text-center mb-12">
                <EditableText
                  isEditable={isEditable}
                  value={config.products.sectionTitle || 'Loft Collections'}
                  onChange={(val) => onUpdateConfig('products', 'sectionTitle', val)}
                  tagName="h2"
                  className="text-3xl font-bold uppercase tracking-wider"
                />
                <div className="w-12 h-[2px] mx-auto mt-4" style={{ backgroundColor: accentColor }}></div>
              </div>
                ...
                {activeLoftProducts.slice(0, 4).map((product, i) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="p-5">
                        <span className="text-[9px] uppercase font-bold tracking-widest text-gray-400 mb-1.5 block">{product.category || 'Decor'}</span>
                        <EditableText
                          isEditable={isEditable}
                          value={product.name}
                        ...
                        <div className="mt-2 text-[10px] text-gray-500 font-light">
                          {(product.specs || []).slice(0, 2).map((sp, sIdx) => (
                            <div key={sIdx} className="flex justify-between py-0.5 border-b border-gray-50">
                              ...
                              <span className="font-semibold text-gray-700">{sp.split(':')[1]}</span>
                            </div>
                          ))}
                        </div>
                      </div>
      ...
      case 'gallery':
        const galleryTitle = config.gallery?.title || 'Design Portfolios';
      ...
      case 'faq':
        const faqTitle = config.faq?.title || 'Care & Delivery Policies';
        ...
        return (
          <SectionWrapper key="faq" isEditable={isEditable} sectionKey="faq" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section className="max-w-4xl mx-auto px-8 py-20 text-[#2E302F]">
              <div className="text-center mb-12">
                <EditableText
                  isEditable={isEditable}
                  value={faqTitle}
      ...
      case 'testimonials':
        const testTitle = config.testimonials?.title || 'Design Appreciations';
        ...
        return (
          <SectionWrapper key="testimonials" isEditable={isEditable} sectionKey="testimonials" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section className="bg-gray-100 text-[#2E302F] py-24">
      ...
      case 'hours':
        const hoursTitle = config.hours?.title || 'Visit Our Studio';
        ...
        return (
          <SectionWrapper key="hours" isEditable={isEditable} sectionKey="hours" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section className="max-w-xl mx-auto px-8 py-20 text-[#2E302F] text-center">
              ...
              <div className="divide-y divide-gray-100 border-t border-b border-gray-100 py-4">
                {hoursDays.map((item, idx) => (
                  <div key={idx} className="flex justify-between py-3.5 text-xs tracking-wider">
                    ...
                    <span className="font-bold text-[#2E302F]">{item.hours}</span>
                  </div>
                ))}
              </div>
            </section>
      ...
      case 'contact':
        const contactTitle = config.contact?.title || 'Connect With Us';
        const contactSubtitle = config.contact?.subtitle || 'Get in touch for custom designer consultations.';
        return (
          <SectionWrapper key="contact" isEditable={isEditable} sectionKey="contact" sectionOrder={order} onUpdateConfig={onUpdateConfig} config={config}>
            <section id="contact" className="max-w-4xl mx-auto px-8 py-24 text-[#2E302F]">
                ...
                <div className="bg-[#fcfcfc] p-8 border border-gray-200 rounded-none shadow-2xl relative">
                  {formSubmitted ? (
                    ...
                    <form onSubmit={handleContactSubmit} className="space-y-5">
                      ...
                      <div>
                        <label className="block text-[9px] uppercase tracking-wider text-gray-400 mb-1.5">Your Message / Request</label>
                        <textarea 
                          rows={3}
                          required
  ...
  if (currentPage === 'shop') {
    return (
      <div 
        ...
        className="w-full h-full overflow-y-auto relative bg-[#ffffff] text-[#2E302F] flex flex-col justify-between font-sans"
      >
        {/* Simple Header */}
        <nav className="py-6 px-8 border-b border-gray-150 flex items-center justify-between">
          {isEditable ? (
            <EditableText
              ...
              tagName="span"
              className="font-bold text-xl uppercase tracking-wider text-[#2E302F] cursor-text"
            />
          ) : (
            <button onClick={() => changePage('home')} className="font-bold text-xl uppercase tracking-wider text-[#2E302F]">
              {businessName}
            </button>
          )}
          <button 
            onClick={() => changePage('home')}
            className="text-[10px] uppercase tracking-wider font-bold border border-[#2E302F] px-4 py-2 transition-all hover:bg-slate-100"
          >
            ← Back To Store
          </button>
        </nav>
            ...
            {sortedFilteredProducts.map((product, i) => (
              ...
              >
                <div>
                  ...
                  <div className="p-5">
                    <span className="text-[9px] uppercase font-bold text-gray-400 tracking-wider mb-1 block">{product.category || 'Decor'}</span>
                    <EditableText
                      isEditable={isEditable}
                      value={product.name}
                      onChange={(val) => onUpdateProduct(product._id || product.id, 'name', val)}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          {isEditable ? (
            <EditableText
              ...
              onChange={(val) => onUpdateConfig('navbar', 'logoText', val)}
              tagName="span"
              className={`font-bold text-xl uppercase tracking-widest cursor-text ${(scrolled || currentPage !== 'home') ? 'text-[#2E302F]' : 'text-white'}`}
            />
          ) : (
            <button onClick={() => changePage('home')} className={`font-bold text-xl uppercase tracking-widest focus:outline-none ${(scrolled || currentPage !== 'home') ? 'text-[#2E302F]' : 'text-white'}`}>
              {businessName}
            </button>
          )}
          ...
          <div className="flex items-center gap-6">
            {[
              ...
              <button 
                ...
                className={`text-xs font-bold uppercase tracking-wider transition-colors hover:opacity-85 ${currentPage === item.id ? 'underline underline-offset-4 font-black' : ''} ${(scrolled || currentPage !== 'home') ? 'text-[#2E302F]' : 'text-white'}`}
              >
                {item.label}
              </button>
            ))}
            ...
            <div className="bg-white rounded-none max-w-lg w-full p-6 border border-gray-200 shadow-2xl relative text-left">
              ...
              <div className="space-y-4 font-sans text-xs text-gray-600">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider text-gray-400 mb-1.5">Room Category</label>
                    <input 
                      type="text"
                ...
                {/* Specs list (Loft materials) */}
                <div>
                  <label className="block text-[10px] uppercase tracking-wider text-gray-400 mb-2">Decor Specifications (Format: Spec: Detail)</label>
                    ...
                    {(product.specs || []).map((sp, idx) => (
                      <div key={idx} className="flex gap-2 items-center">
                        ...
                        <button 
                          ...
                          className="px-2.5 py-1.5 bg-red-50 hover:bg-red-100 text-red-650 border border-red-200 text-xs font-bold"
                        >
                          Delete
                        </button>
                      </div>
                ...
                <div>
                  <label className="block text-[10px] uppercase tracking-wider text-gray-400 mb-1.5">Stock Quantity</label>
                  <input 
                    type="number"
                    min="0"
                ...
                <div className="flex gap-3 pt-4 border-t border-gray-100 justify-between items-center">
                  <button 
                    ...
                    className="px-4 py-2.5 bg-red-50 hover:bg-red-100 text-red-650 border border-red-200 text-[10px] uppercase font-bold tracking-wider"
                  >
                    Delete Item
                  </button>
                  <button 
          ...
          <div className="bg-white rounded-none max-w-lg w-full p-6 shadow-2xl border border-gray-200 relative text-left">
            ...
            <div className="flex border-b border-gray-200 mb-6 font-semibold">
              ...
              <button 
                ...
                className={`flex-1 py-2 font-bold text-xs uppercase tracking-wider text-center border-b-2 transition-colors ${activeTab === 'ai' ? 'border-[#2E302F] text-[#2E302F]' : 'border-transparent text-gray-400 hover:text-gray-550'}`}
              >
                AI Generator
              </button>
            </div>
            ...
            {activeTab === 'presets' ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[300px] overflow-y-auto p-1">
                {[
                  { name: 'Scandinavian Living Space', url: 'https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?auto=format&fit=crop&w=1200&q=80' },
                  { name: 'Warm Cozy Dining Area', url: 'https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?auto=format&fit=crop&w=1200&q=80' },
                  { name: 'Earthy Decor Ornaments', url: 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=1200&q=80' },
                  { name: 'Sunny Work Desk Setup', url: 'https://images.unsplash.com/photo-1598300042247-d088f8ab3a91?auto=format&fit=crop&w=1200&q=80' },
                  { name: 'Modern Wooden Dresser', url: 'https://images.unsplash.com/photo-1595428774223-ef52624120d2?auto=format&fit=crop&w=1200&q=80' },
                  { name: 'Nordic Pot Collection', url: 'https://images.unsplash.com/photo-1612196808214-b8e1d6145a8c?auto=format&fit=crop&w=1200&q=80' }
                ].map((item, idx) => (
                  <div 
                    key={idx}
                    onClick={() => {
